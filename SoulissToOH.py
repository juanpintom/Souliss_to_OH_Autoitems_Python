...
f2.write('Group:Number:AVG() Temperaturas_Medias "Temperaturas Medias [%.1f Â°C]" <temperature> (weather)'  + '\n')
...
for i in decoded["id"]:
    ...
    f1.write(site.encode('latin1') + '\n')
    line = 'String timestamp_node' + str(nod) + ' "Timestamp ' + ndesc + ' [%1$td.%1$tm.%1$tY %1$tk:%1$tM:%1$tS]" <keyring> (node' + str(nod) + ', diagnostic) {souliss="D99' + ':' + str(nod) + ':999"}' 
    f2.write(line.encode('latin1') + '\n')
    ...
    for j in decoded["id"][nod]["slot"]:
        ...
        #T5X TYPICALS
            ...
            site = 'Text item=node' + str(nod) + '_slot' + slo
            line = 'Number node' + str(nod) + '_slot' + slo + ' "' + ddesc + ' [%.1f A]" <current> (node' + str(nod) + ', current) {souliss="T' + str(typ) + ':' + str(nod) + ':' + slo + '"}' 
        elif (typ == 57):
            site = 'Text item=node' + str(nod) + '_slot' + slo
            line = 'Number node' + str(nod) + '_slot' + slo + ' "' + ddesc + ' [%.1f W]" <watt> (node' + str(nod) + ', watt) {souliss="T' + str(typ) + ':' + str(nod) + ':' + slo + '"}' 
        elif (typ == 58):
            site = 'Text item=node' + str(nod) + '_slot' + slo
